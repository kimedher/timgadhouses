...
The identifier system is designed to be stable, location-based, and independent of excavation-era labels, which are inconsistent across a century of French reports. Each insula gets coordinates. Each house gets a two-digit suffix assigned in a consistent order within its insula. Full documentation of the system, including edge cases and how it reconciles with earlier labels, will live on the [Methods](/methods/) page as the site grows. For now, the short version is that the identifier tells you where the house is, not when it was excavated or by whom.
